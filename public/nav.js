...
function updateCreateLink() {
  const createLinks = document.querySelectorAll("a.create-btn");
  if (!createLinks.length) return;

  const urlParams = new URLSearchParams(window.location.search);
  const nestId = urlParams.get("nest");
  const href = nestId
    ? `/create/index.html?nest=${encodeURIComponent(nestId)}`
    : "/create/index.html";

  createLinks.forEach((link) => link.setAttribute("href", href));
}
